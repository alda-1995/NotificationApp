    ...
    <div class="py-3">
        <div class="flex overflow-hidden rounded-xl border border-[#dbe1e6] bg-white">
            <table class="flex-1">
                <thead>
                    <tr class="bg-white">
                        <th class="px-4 py-3 text-left text-[#111518] w-[400px] text-sm font-medium leading-normal">Nombre</th>
                        <th class="px-4 py-3 text-left text-[#111518] w-[400px] text-sm font-medium leading-normal">Tipo</th>
                        <th class="px-4 py-3 text-left text-[#111518] w-[400px] text-sm font-medium leading-normal">Fecha de envío</th>
                        <th class="px-4 py-3 text-left text-[#60768a] w-60 text-sm font-medium leading-normal">Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($notifications as $notification)
                        <tr class="border-t border-t-[#dbe1e6]">
                            <td class="h-[72px] px-4 py-2 w-[400px] text-[#111518] text-sm font-normal leading-normal">
                                {{ $notification->title }}
                            </td>
                            <td class="h-[72px] px-4 py-2 w-[400px] text-[#60768a] text-sm font-normal leading-normal">
                                {{ ucfirst($notification->channel) }}
                            </td>
                            <td class="h-[72px] px-4 py-2 w-[400px] text-[#60768a] text-sm font-normal leading-normal">
                                {{ \Carbon\Carbon::parse($notification->send_at)->format('Y-m-d') }}
                            </td>
                            <td class="h-[72px] px-4 py-2 w-60 text-[#60768a] text-sm font-bold leading-normal tracking-[0.015em]">
                                <a href="#" class="hover:underline">Ver detalle</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-6 text-[#60768a]">No hay notificaciones registradas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
