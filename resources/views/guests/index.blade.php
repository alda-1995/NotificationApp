...
@section('content')
    <div class="flex flex-col max-w-[960px]">
        <div class="flex flex-wrap justify-between gap-3 py-4">
            ...
            <x-controls.button href="{{ route('guests.create') }}" variant="add">
                Añadir invitado
            </x-controls.button>
        </div>
        <div class="py-3">
            <label class="flex flex-col min-w-40 h-12 w-full">
                <div class="flex w-full flex-1 items-stretch rounded-xl h-full">
                    ...
                    <input placeholder="Buscar invitados"
        ...
        <div class="py-3">
            <div class="flex overflow-hidden rounded-xl border border-[#dde1e3] bg-white">
                <table class="flex-1">
                    <thead>
                        <tr class="bg-white">
                            <th class="px-4 py-3 text-left text-[#121416] w-[400px] text-sm font-medium leading-normal">
                                Nombre</th>
                            <th class="px-4 py-3 text-left text-[#121416] w-[400px] text-sm font-medium leading-normal">
                                Correo</th>
                            <th class="px-4 py-3 text-left text-[#121416] w-[400px] text-sm font-medium leading-normal">
                                Teléfono</th>
                            <th class="px-4 py-3 text-left text-[#121416] w-60 text-sm font-medium leading-normal">
                                Preferencias</th> <!-- Nueva columna -->
                            <th class="px-4 py-3 text-left text-[#121416] w-60 text-sm font-medium leading-normal">Acciones
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($guests as $guest)
                            <tr class="border-t border-t-[#dde1e3]">
                                ...
                                <td
                                    class="h-[72px] px-4 py-2 w-60 text-[#6a7681] text-sm font-bold leading-normal tracking-[0.015em]">
                                    <a href="{{ route('guests.show', $guest->guest_id) }}" class="hover:underline">Ver
                                        detalle</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-4 text-[#6a7681]">No hay invitados registrados.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
